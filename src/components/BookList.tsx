...
function StatusBadge({ book, onUpdated }: { book: Book; onUpdated: (id: string, status: string) => void }) {
  ...
  async function changeStatus(newStatus: string) {
    if (newStatus === book.status || loading) return;
    setLoading(true);
    setOpen(false);
    await updateBook(book.id, { status: newStatus as Book["status"] });
    onUpdated(book.id, newStatus);
    setLoading(false);
  }
  ...
  return (
    <div ref={ref} className="absolute right-1.5 top-1.5 z-10">
      <button
        ...
        className={`rounded-full px-2 py-0.5 text-[10px] font-medium transition-opacity ${STATUS_COLORS[book.status]} ${loading ? "animate-pulse" : "hover:opacity-80"}`}
      >
        {STATUS_LABELS[book.status]}
      </button>
      {open && (
        <div className="absolute right-0 top-full mt-1 min-w-[5rem] overflow-hidden rounded-lg border border-border bg-card shadow-lg">
          {statuses.map((s) => (
            <button
              key={s}
              ...
              className={`block w-full px-3 py-1.5 text-left text-xs transition-colors hover:bg-muted ${
                s === book.status ? "font-bold text-primary" : "text-foreground"
              }`}
            >
              {STATUS_LABELS[s]}
            </button>
              ...
              >
                ...
                >
                  <div className="relative mb-3 aspect-[2/3] overflow-hidden rounded-lg bg-muted">
                    ...
                    <StatusBadge book={book} onUpdated={handleStatusUpdated} />
                  </div>
